...
class Exp_Short_Term_Forecast(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if 'Gym' not in self.args.model:
            ...
            if getattr(self.args, 'bfloat16', False):
        ...
        if os.path.exists(best_model_path) and False:
            ...
            for epoch in range(self.args.train_epochs):
                ...
                for i, batch_data in enumerate(train_loader):
                    ...
                    if getattr(self.args, 'bfloat16', False):
                        batch_x = batch_x.bfloat16().to(self.device)
                        batch_y = batch_y.bfloat16().to(self.device)
                        batch_y_mark = batch_y_mark.bfloat16().to(self.device)
                        dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).bfloat16()
                        dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).bfloat16().to(self.device)
                    else:
    ...
    def vali(self, train_loader, vali_loader, criterion):
        ...
        # Use vali_loader timeseries (now correctly sampled to match train_loader)
        y = vali_loader.dataset.timeseries
        x = torch.tensor(x, dtype=torch.bfloat16 if getattr(self.args, 'bfloat16', False) else torch.float32).to(self.device)
        x = x.unsqueeze(-1)
        ...
        with torch.no_grad():
            # decoder input
            B, _, C = x.shape
            dec_inp = torch.zeros((B, self.args.pred_len, C)).to(self.device)
            if getattr(self.args, 'bfloat16', False):
                dec_inp = dec_inp.bfloat16()
                dec_inp = torch.cat([x[:, -self.args.label_len:, :], dec_inp], dim=1).bfloat16()
            else:
                dec_inp = dec_inp.float()
                dec_inp = torch.cat([x[:, -self.args.label_len:, :], dec_inp], dim=1).float()
            # encoder - decoder
            outputs = torch.zeros((B, self.args.pred_len, C))
            if getattr(self.args, 'bfloat16', False):
    ...
    def test(self, setting, test=0):
        ...
        x, _ = train_loader.dataset.last_insample_window()
        # Use test_loader timeseries (now correctly sampled to match train_loader)
        ...
        with torch.no_grad():
            ...
            if getattr(self.args, 'bfloat16', False):
                ...
                dec_inp = torch.cat([x[:, -self.args.label_len:, :], dec_inp], dim=1).float()
            # encoder - decoder
            outputs = torch.zeros((B, self.args.pred_len, C)).to(self.device)
            if getattr(self.args, 'bfloat16', False):
                outputs = outputs.bfloat16()
            else:
                outputs = outputs.float()
        ...
        if self.args.ensemble_mode:
            if 'TSGym' in setting:
                if 'Transformer' in setting:
                    folder_path = f'./results_short_term_forecasting_ensemble/results{self.save_suffix}_transformer/{dataset}/' + setting + '/'
                elif 'LLM' in setting:
                    folder_path = f'./results_short_term_forecasting_ensemble/results{self.save_suffix}_LLM/{dataset}/' + setting + '/'
                elif 'TSFM' in setting:
                    folder_path = f'./results_short_term_forecasting_ensemble/results{self.save_suffix}_TSFM/{dataset}/' + setting + '/'
                elif 'MLP' in setting:
                    folder_path = f'./results_short_term_forecasting_ensemble/results{self.save_suffix}_MLP/{dataset}/' + setting + '/'
                elif 'GRU' in setting:
                    folder_path = f'./results_short_term_forecasting_ensemble/results{self.save_suffix}_GRU/{dataset}/' + setting + '/'
            else:
                folder_path = f'./results_short_term_forecasting_ensemble/results{self.save_suffix}/{dataset}/' + setting + '/'

        if not os.path.exists(folder_path):
            os.makedirs(folder_path)

        forecasts_df = pandas.DataFrame(preds[:, :, 0], columns=[f'V{i + 1}' for i in range(self.args.pred_len)])
        # Use test_loader IDs (now correctly sampled to match train_loader)
        ...
        if self.args.ensemble_mode:
             np.save(folder_path + f'{self.args.seasonal_patterns}_pred.npy', preds)
             np.save(folder_path + f'{self.args.seasonal_patterns}_true.npy', trues)
