...
def get_data(df_raw, data_name,seq_len,flag='train'):
...
def get_meta_faetures(file_paths=None, meta_feature_type='tsfel',save_path=None):
    ...
    if meta_feature_type == 'tsfel':
        meta_features = {}
        for _ in tqdm(file_paths, desc="[TSFEL] Extracting meta features"):
            start = time.time()
            meta_features[_.split('/')[-1].split('.')[0]] = get_meta_feature_tsfel(_)
            print(f"  {_.split('/')[-1]}: {time.time() - start:.2f} seconds")
        ...
        meta_features = {}
        for _ in tqdm(file_paths, desc="[TSFEL-GRP] Extracting meta features"):
            start = time.time()
            meta_features[_.split('/')[-1].split('.')[0]] = get_meta_feature_tsfel(_, target_dim=256)
            print(f"  {_.split('/')[-1]}: {time.time() - start:.2f} seconds")
        save_path = save_path if save_path is not None else "./meta_feature_dict_tsfelGRP.npz"
    elif meta_feature_type == 'tsfused':
        meta_features = {}
        for _ in tqdm(file_paths, desc="[TSFused] Extracting meta features"):
            start = time.time()
            meta_features[_.split('/')[-1].split('.')[0]] = get_meta_feature_tsfused(_)
            print(f"  {_.split('/')[-1]}: {time.time() - start:.2f} seconds")
        save_path = save_path if save_path is not None else "./meta_feature_dict_tsfused.npz"
    elif meta_feature_type == 'tabpfn':
        meta_features = {}
        for _ in tqdm(file_paths, desc="[TabPFN] Extracting meta features"):
            start = time.time()
            meta_features[_.split('/')[-1].split('.')[0]] = get_tabpfn_embedding(_)
            print(f"  {_.split('/')[-1]}: {time.time() - start:.2f} seconds")
    ...
    print(f"\n[Total] Extracting meta features for {len(meta_features)} datasets took: {time.time() - total_start:.2f} seconds")
