...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        # Padding for prediction
        zeros = torch.zeros(B, self.pred_len, M).to(x_enc.device)
        x_enc = torch.cat([x_enc, zeros], dim=1)

        # Ensure divisibility by patch_len
        if x_enc.shape[1] % self.patch_len != 0:
            pad_len = self.patch_len - (x_enc.shape[1] % self.patch_len)
            zeros_pad = torch.zeros(B, pad_len, M).to(x_enc.device)
            x_enc = torch.cat([x_enc, zeros_pad], dim=1)
        ...
        # Slice to get prediction duration
