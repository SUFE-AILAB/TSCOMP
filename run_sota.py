...
def create_task_list(param_devices, env, dataset, singlemodel=None, verbose=False):
    ...
    if env == 'mqenv' or env == 'base':
        ...
        if singlemodel is not None:
            ...
            model_list += ['DUET', 'RAFT', 'OLinear']#, 'CrossCrossModel'
            model_list += ['Timer', 'TimeLLM', 'Moment', 'GPT4TS', 'Chronos', 'TimeMOE'] # LLM模型
    elif env =='mamba':
         # 在虚拟环境mamba中运行
    ...
    for i in range(len(df)):
        for model_name in model_list:
            ...
            if 'Mamba' in model_name:
                ...
                # 仿照已有sota设置参数，使得训练效率更高 
                d_model, d_ff = 32, 128 
            elif model_name == 'Moment' or model_name == 'Chronos' or model_name == 'Timer' or model_name == 'TimeMOE':
                # 仿照已有sota设置参数，使得训练效率更高 
                d_model, d_ff = 64, 256
            elif 'TSGym' in model_name:
                # 仿照已有sota设置参数，使得训练效率更高 
                d_model, d_ff = 64, 256
            ...
            if model_name in ['Timer', 'TimeLLM', 'Moment', 'GPT4TS', 'RAFT', 'DUET', 'Crossformer', 'Chronos', 'TimeMOE']:
                accumulation_steps = 2

            # Auto-set few-shot ratio for large models
            few_shot_ratio = 0
            if model_name in ['Timer', 'TimeLLM', 'Moment', 'GPT4TS', 'Chronos', 'TimeMOE']:
                few_shot_ratio = 0.05
            ...
            if 'M4' in data_name:
                ...
                expand_str = ""
                if model_name in ['Timer', 'TimeMOE']:
                    expand_str = " --frozen 0"
                ...
                task_command = f"""CUDA_VISIBLE_DEVICES={param_devices} python3 -u run.py \
                        ...
                        --d_ff {d_ff} \
                        --loss {loss} \
                        --batch_size {16 // accumulation_steps} \
                        --learning_rate 0.001 \
                ...
                for ii, pred_len in enumerate(pred_len_list):
                    ...
                    # Ili数据集预测长度不同
                    if data_name in ['ili', 'covid-19', 'fred-md', 'nyse', 'nasdaq', 'nn5', 'wike2000'] :
                        pred_len = ili_pred_len_list[ii]
                        seq_len = 36
                        label_len = 18 # seq_len的一半
                        # SegRNN seq_len、label_len、pred_len必须是seg_len的整数倍
                        seg_len = 12
                        # Timer uses patch_len instead of seg_len
                        patch_len = 12
                        ...
                        if model_name == 'Koopa':
                            ...
                            seq_len = 512
                        elif model_name in ['Timer', 'TimeMOE']:
                            # Timer requires seq_len = pred_len to produce correct output length
                            seq_len = pred_len
                    ...
                    # Timer and TimeMOE use patch_len instead of seg_len
                    if model_name in ['Timer', 'TimeMOE']:
                        len_param_name = 'patch_len'
                        len_param_value = patch_len
                    else:
                        len_param_name = 'seg_len'
                        len_param_value = seg_len
                    ...
                    task_command = f"""CUDA_VISIBLE_DEVICES={param_devices} python3 -u run.py \
                            ...
                            --label_len {label_len} \
                            --pred_len {pred_len} \
                            --{len_param_name} {len_param_value} \
                            --e_layers {e_layers} \
                            --d_layers {d_layers} \
                            --factor 3 \
                            --enc_in {data_fea_num} \
...
def main():
    ...
    parser.add_argument('--n_parts', type=int, default=1, help='total parts')
    parser.add_argument('--dry_run', type=int, default=0, help='if 1, do not run')
    args = parser.parse_args()
    
    task_list = create_task_list(args.devices, args.env, args.dataset, args.singlemodel, args.verbose)

    # Split task_list if needed
    if args.n_parts > 1:
        total_tasks = len(task_list)
        chunk_size = (total_tasks + args.n_parts - 1) // args.n_parts
        start_idx = args.part * chunk_size
        end_idx = min(start_idx + chunk_size, total_tasks)
        task_list = task_list[start_idx:end_idx]
        print(f"Running part {args.part + 1}/{args.n_parts}, tasks {start_idx + 1} to {end_idx} (Total {total_tasks})")

    if args.dry_run:
        print("Dry run mode. Tasks to be executed:")
